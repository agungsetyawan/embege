...
- UI: shadcn components in the Base-UI generation (components.json style `base-nova`), ReUI free components in `src/components/reui` (frame, badge, timeline, alert, icon-tile, icon-stack, cascader, autocomplete) installed via CLI only. Admin surfaces use ReUI Frame, public surfaces stay shadcn Card. Lucide icons. `react-day-picker` v10 with the `date-fns` Indonesian locale.
