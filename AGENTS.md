...
- UI: shadcn components in the Base-UI generation (components.json style `base-nova`), ReUI free components in `src/components/reui` (frame, badge, alert, icon-tile, icon-stack, cascader, data-grid) installed via CLI only. Admin surfaces use ReUI Frame, public surfaces stay shadcn Card. Recharts for the trend chart (`ui/chart`); `ui/tabs` for the Daftar/Grafik and Kasus/Korban toggles. Lucide icons. `react-day-picker` v10 with the `date-fns` Indonesian locale.
