...
The admin dashboard lists pending items with an AI summary and a location guess for each item. A debounced search box and a district filter narrow the queue across the Antrean, Ditolak, Laporan, and Terhapus tabs. The admin picks the district, edits the summary, then approves, rejects, or — for duplicate-flagged items — applies it as an update to the linked published case. Approved items appear on the public map. A separate `/admin/cases` page lists all cases in a filterable table where the admin can edit, soft-delete, and restore rows; each row links its per-case action history.
